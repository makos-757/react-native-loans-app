...
export type UnauthorizedHandler = (context: { hasToken: boolean; errorData: unknown }) => void;
...
let _refreshTokenGetter: AuthTokenGetter | null = null;
let _setTokens: ((access: string, refresh?: string) => Promise<void>) | null = null;
...
export function setRefreshTokenGetter(getter: AuthTokenGetter | null): void {
  _refreshTokenGetter = getter;
}

export function setSetTokens(fn: (access: string, refresh?: string) => Promise<void>): void {
  _setTokens = fn;
}
...
export async function customFetch<T = unknown>(
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      if (_onUnauthorized) {
        _onUnauthorized({ hasToken, errorData });
      }
    }
...
async function attemptTokenRefresh(): Promise<boolean> {
  ...
  _refreshPromise = (async () => {
    try {
      const storedRefresh = await _refreshTokenGetter?.();
      if (!storedRefresh) return false;
      const result = await refreshAccessToken(storedRefresh);
      if (result) {
        await _setTokens?.(result.token, result.refreshToken);
        return true;
      }
      return false;
    } finally {
